...
export type RoutePath = "/login" | "/register" | "/home" | "/swipe" | "/profile" | "/admin" | `/admin/${string}` | `/museums/${string}`;
...
function normalize(hash: string): RoutePath {
  ...
  if (path.startsWith("/admin/") && path.replace("/admin/", "").trim()) return path as RoutePath;
